...
value_of_foo = foo._get()  # this method will check for the environment variable FOO, and parse it as an int
...
foo._get()  # now we will parse the value of FOO as a JSON list, and ensure that all its inner values are numbers
...
service_connection_settings: ConnectionSetting = connection_settings._get()
